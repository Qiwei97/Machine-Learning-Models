             ...
             tabPanel("Visualisations", value = "panel4",
                      ...
                      fluidPage(value = "panel4",
                                navbarPage(id = "navbar2", "Select a chart:",
                                           ...
                                           #tab for ratings graph
                                           tabPanel("Feature Importance Plot",
                                                    plotOutput('mlplot'),
                                                    br(),
                                                    fluidRow(column(12, 
                                                                    "This feature importance plot is generated by our in-house AI.
                                                                    It shows the relative importance of each variable in predicting the ratings of 
                                                                    a particular product. It is expected that different products will have different plots."))),
                                           
                                           
                                           #tab for ratings graph
                                           tabPanel("Analytics",
                                                    plotOutput('ratingplot2'),
                                                    selectInput('ratingplottype','Graph',c("Price vs Ratings", "Price vs Units Sold", "HeatMap (Price)","HeatMap (Units Sold)","HeatMap (Ratings)"))
                                           )
                                )
...
server <- function(input, output, session){
  ...
  wordcloudInput <- reactive({
    create_wordcloud(data2 %>% filter(Product == as.character(item())) %>% filter(Platform == as.character(input$cloudweb)) %>% select(Review),
  ...
  rating <- reactive({
    if (as.character(input$ratingweb) == 'all'){
      
      data3 <- data %>% filter(Product == as.character(item())) %>% filter(Rating != 0)
      
      ggplot(data=data3, aes(x =Rating)) + geom_density() + theme_economist() + scale_fill_economist() + ylab('Count') + xlab('Rating')  + ggtitle('Rating distribution')
      
    } else if (as.character(input$ratingweb) == 'all 4'){
      
      data3 <- data %>% filter(Product == as.character(item())) %>% filter(Rating != 0)
      
      ggplot(data=data3, aes(x =Rating)) + geom_density() + facet_wrap(~Platform) + theme_economist() + scale_fill_economist() + ylab('Count') + xlab('Rating')  + ggtitle('Rating distribution')
      
    } else {
      
      data3 <- data %>% filter(Product == as.character(item())) %>% filter(Platform == as.character(input$ratingweb)) %>% filter(Rating != 0)
  ...
  ratingplot <- reactive({
    ...
    if (input$ratingplottype == 'Price vs Ratings'){
      ggplot(data=data3) + geom_point(aes(x=Price, y=Rating)) + geom_smooth(aes(x=Price,y=Rating), method = "lm", color= 'red')
    } else if (input$ratingplottype == 'Price vs Units Sold'){
      ggplot(data=data3, aes(x=Price, y=Units.Sold, fill=Rating)) + geom_point(colour='blue', aes(size=Rating)) + geom_smooth(method = "lm", color= 'red')
    } else if (input$ratingplottype == 'HeatMap (Price)'){
    ...
    } else if (input$ratingplottype == 'HeatMap (Units Sold)'){
      ggplot(data, aes(Product, Platform)) + 
        geom_tile(aes(fill= Units.Sold)) + scale_fill_gradient(low='white',high='blue') + theme(axis.text.x=element_text(angle=90))
    } else {
      ggplot(data, aes(Product, Platform)) + geom_tile(aes(fill= Rating)) + scale_fill_gradient(low='white',high='blue') + theme(axis.text.x=element_text(angle=90))
    }
  })
